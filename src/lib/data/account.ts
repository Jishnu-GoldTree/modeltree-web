...
const ORDER_FIXTURES: Record<string, { slug: string; id: string; placedOn: string }[]> = {
  u_alex: [
...
const FAVORITE_FIXTURES: Record<string, string[]> = {
  u_alex: [
    "character-fantasy-knight",
    "animal-grey-wolf",
    "food-ramen-bowl",
    "aircraft-airliner",
  ],
  u_kamara: ["character-street-runner", "animal-barn-owl", "car-rally-car"],
}
...
export function getFavorites(userId: string): CatalogModel[] {
  return (FAVORITE_FIXTURES[userId] ?? [])
...
export function getAccountStats(userId: string) {
  ...
  return {
    ...
    favorites: getFavorites(userId).length,
    downloads: orders.reduce((sum, order) => sum + order.model.formats.length, 0),
  }
}
