      ...
      <main className="flex-1 pt-16">
        <div className="shell py-10">
          <div className="flex flex-wrap items-baseline justify-between gap-3">
            <div>
              <h1 className="text-2xl font-semibold tracking-tight">Saved models</h1>
              {models.length > 0 && (
          ...
          {models.length === 0 ? (
            <div className="mt-8 flex flex-col items-center rounded-xl border py-20 text-center">
              ...
              <p className="mt-1.5 max-w-sm text-sm text-muted-foreground">
                Tap the heart on any model to keep it here while you compare
                options. Saved models stay for six months, signed in or not.
              </p>
              <Button
                asChild
            ...
            <ul className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-4">
              {models.map((model) => {
                const [standard] = getLicenseOptions(model)
                return (
